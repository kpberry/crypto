...
class Communicator:
    ...
    def generate_key(self, recipient, n=256):
        ...
        '''
        a = n_bit_rand_int(n)
        A = mod_exp(self.g, a, self.p)  # g ^ a mod p
        B = self.request_public_key(recipient, n)
        self.send_public_key(A, recipient)
        key = mod_exp(B, a, self.p)  # B ^ a mod p == (g ^ b) ^ a == g ^ ab
    ...
    def receive_public_key_request(self, n):
        ...
        '''
        b = n_bit_rand_int(n)
        B = mod_exp(self.g, b, self.p)  # g ^ b mod p
        self.private_key = b
        return B

    def send_public_key(self, public_key, recipient):
        '''
        Replace with network code to make this code actually work for
        clients/servers
    ...
    def receive_public_key(self, public_key, sender):
        ...
        '''
        self.keys[sender] = mod_exp(public_key, self.private_key, self.p)  # A ^ b mod p == (g ^ a) ^ b == g ^ ab
    ...
    def compute_hash(message):
        '''
        Compute a SHA-3 hash for a string message to use when authenticating
        :param message: the message whose hash will be computed
